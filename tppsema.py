    ...
    for declaracao in semTable:
        if declaracao['declaration'] == 'chamada_funcao':
            ...
            if func_id not in funcoesDeclaradas:
                arrError.append(error_handler.newError(showKey, 'ERR-SEM-CALL-FUNC-NOT-DECL', data=func_id))
...
def verificarFuncoesNaoUtilizadas(semTable):
    ...
    for func in funcoesDeclaradas:
        if func not in funcoesUtilizadas:
            arrError.append(error_handler.newError(showKey, f"WAR-SEM-FUNC-DECL-NOT-USED", data=func))
...
def verificarTipoRetorno(semTable):
    ...
    for declaracao in semTable:
        ...
        if declaracao['declaration'] == PalavrasChaves.declaracao_funcao.value:
            funcao_id = declaracao.get('id', '')
            tipoDaFuncao = declaracao.get('type', '')
            ...
            if not mesmoRetorno or not achouRetorno:
                # print(funcao_id)
                # print(tipoDaFuncao)
                             
                teste = [funcao_id, tipoDaFuncao, "vazio"]
                arrError.append(error_handler.newError(showKey, 'ERR-SEM-FUNC-RET-TYPE-ERROR', data=teste))
...
def find_parent_type(node):
    # Inicialmente, o pai é o nó atual
    current_node = node
    
    # Percorre os pais até encontrar o tipo desejado
    while current_node is not None:
        if current_node.type:
            return current_node.type
        current_node = current_node.parent
    
    # Retorna None se nenhum tipo for encontrado
    return None
    ...
    for declaracao in semTable:
        ...
        if declaracao['declaration'] == 'declaracao_variaveis':     
            ...
            if(var_id in variaveisDeclaradas[scope]):
                tipoDaFuncao = declaracao['type']
                if tipoDaFuncao == "FLUTUANTE":
                    tipo_do_negocio = "inteiro"
                if tipoDaFuncao == "INTEIRO":
                    tipo_do_negocio = "flutuante"
                teste=[var_id, tipo_do_negocio]
                arrError.append(error_handler.newError(showKey,'WAR-SEM-VAR-DECL-PREV', data=teste))
        ...
        for var in vars_inicializadas:
            if var not in vars_declaradas:
                if(var not in variaveisNaoDeclarada):
                    arrError.append(error_handler.newError(showKey,'ERR-SEM-VAR-NOT-DECL', data=var))
        ...
        for var in vars_declaradas:
            if var not in vars_declaradasEUtil:
                if(var not in variaveisDeclaradasENaoUtilizada):
                    ...
                    arrError.append(error_handler.newError(showKey,'WAR-SEM-VAR-DECL-NOT-USED', data=var))
            else:
                ...
                if(var not in vars_declaradasEInit):
                    if( var not in variaveisDeclaradasENaoInicializada):
                        ...
                        arrError.append(error_handler.newError(showKey,'WAR-SEM-VAR-DECL-NOT-INIT', data=var))

            # Prints para cada estrutura de dados
            ...
            # print(variaveisNaoDeclarada)
            # print("Variáveis Declaradas e Utilizadas:")
            # print(variaveisDeclaradasEUtilizadas)
            # print("Variáveis Declaradas e Inicializadas:")
            # print(variaveisDeclaradasEInicializadas)
            # print("Variáveis Declaradas e Não Utilizadas:")
            # print(variaveisDeclaradasENaoUtilizada)
            # print("Variáveis Declaradas e Não Inicializadas:")
